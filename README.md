...
- **项目概述**：该平台面向知识获取与分享社区场景，由后端 Spring Boot 实现用户认证、知文发布、Feed 流展示、全文搜索等功能；结合DeepSeek AI摘要生成、RAG知识库等技术，支持多用户在线知识创作、分享与智能问答，并针对高并发高可用场景进行缓存、异步、限流等多维度优化。
...
- **项目细节与亮点**：
  ...
  - **异步链路**：采用 outbox + Canal + Kafka 模式实现业务主链路与下游服务解耦，链接用户关系落库、ES索引增量同步、计数更新等下游服务，并做对不同业务场景作幂等性、顺序消费等处理，提高业务接口相应速度与QPS。
