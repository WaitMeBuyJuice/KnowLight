...
- **项目概述**：该平台面向用户社交和知识交流的社区场景，具备用户认证与社交、知文管理、Feed 流推荐、全文搜索等功能；结合LLM知文总结、RAG 知识问答技术，支持多用户在线实时创作与互动，并针对 C 端高并发读、突发热点等场景，进行多级缓存、事件分发、并发控制等多维度优化，保障高可用与用户体验。
...
- **项目细节与亮点**：
  - **用户认证系统**：基于 Spring Security 开发 JWT 双令牌认证系统，采用 RS256 签名 + 分布式锁 + Redis 刷新令牌白名单，支持即时令牌撤销，兼顾高并发安全与高性能。
  - **计数系统**：以 Redis SDS 作为底层结构存储知文/用户计数，采用 Hash 聚合计数事件 + 定时Lua脚本 两阶段写入SDS，并作异常重建、降低兜底等处理，应对高并发场景，QPS达8K+。
  - **发布系统**：采用渐进式发布流程，由状态机记录知文发布状态，以预签名+前端直传的形式，上传知文内容文件至 MinIO ，并接入 DeepSeek AI 一键生成文章摘要。
  - **事件分发架构**：采用 outbox + Canal + Kafka 模式实现业务主链路与下游服务解耦，链接用户关系落库、ES索引增量同步、计数更新等下游服务，并做对不同业务场景作幂等性、顺序消费等处理，提高业务接口相应速度与QPS。
  - **Feed 流**：基于Redis + Caffeine 构建四级缓存架构与缓存一致性策略，采用游标分页保证深分页稳定性；针对高并发场景，设计hotkey 探测机制，基于热点检测按层级延长缓存时长，叠加随机抖动抗雪崩；采用分布式锁避免同页并发回源风暴。
  - **搜索系统**：基于 Elasticsearch 构建知文搜索系统，具备关键词检索、低延迟前缀联想、标签过滤，内容高亮等功能，同时通过 function_score 融合 BM25 相关性与点赞等业务权重优化排序，保证结果的相关性。
